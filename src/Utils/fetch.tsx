...
export default async function axiosFetch({
  ...
  const requests: any = {
    ...
    filterMovie: `${baseURL}/discover/movie?language=${language}&sort_by=${sortBy}${year != undefined ? "&year=" + year : ""}${country != undefined ? "&with_origin_country=" + country : ""}&page=${page}&with_watch_providers=8`,
    filterTv: `${baseURL}/discover/tv?language=${language}&sort_by=${sortBy}${year != undefined ? "&first_air_date_year=" + year : ""}${country != undefined ? "&with_origin_country=" + country : ""}&page=${page}&with_networks=213`,
